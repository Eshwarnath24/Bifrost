...
export const SocialIcons = () => (
  <div className="flex space-x-3 mt-4 justify-center w-full">
    <button type="button" className="p-3 flex-1 flex justify-center bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700/50 hover:border-gray-300 dark:hover:border-slate-600 hover:text-gray-900 dark:hover:text-white text-gray-700 dark:text-slate-200 transition-all duration-300 shadow-sm hover:shadow-md active:scale-95">
...
export const GoogleButton = ({ onClick, disabled, isLoading, label = 'Continue with Google' }) => (
  <button type="button" onClick={onClick} disabled={disabled} className="relative overflow-hidden w-full flex items-center justify-center gap-3 bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3.5 text-sm font-bold text-gray-700 dark:text-slate-200 hover:bg-gray-50 dark:hover:bg-slate-700/50 hover:border-indigo-200 dark:hover:border-indigo-500/30 hover:text-indigo-600 dark:hover:text-indigo-400 transition-all duration-300 active:scale-[0.98] shadow-sm hover:shadow-md group disabled:opacity-70 disabled:cursor-not-allowed">
    ...
    {isLoading ? (
      <Loader2 className="w-5 h-5 relative z-10 animate-spin text-indigo-600 dark:text-indigo-400" />
    ) : (
    ...
    <span className="relative z-10">{isLoading ? 'Connecting...' : label}</span>
  </button>
);
